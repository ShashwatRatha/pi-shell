...
int parse(char *cmdstr, Commands *commands, Delims *dlms) {
  ...
  while (tok_idx < toks.count) {
    ...
    cmds[cmd_idx].argv =
        argv; // errors in here will be dealt with by the exec block
    ...
    while (t == REDIR_IN || t == REDIR_OUT_OW || t == REDIR_OUT_APP) {
      ...
      if (t == REDIR_IN) {
        ...
        if ((cmds[cmd_idx].outstream =
                 open(name, O_WRONLY | O_TRUNC | O_CREAT, 0644)) == -1) {
          perror("pish");
          freeCommands(commandFreeForm);
          return EXIT_FAILURE;
        }
        ...
        if ((cmds[cmd_idx].outstream =
                 open(name, O_WRONLY | O_CREAT | O_APPEND, 0644)) == -1) {
          perror("pish");
          freeCommands(commandFreeForm);
          return EXIT_FAILURE;
        }
  ...
  *commands = (Commands){.cmds = cmds, .count = cmd_idx};

  *dlms = (Delims){.delims = delims, .count = delim_idx};

  return SUCCESS;
}

static inline char isDelim(TokenType tp) {
  return ((tp == PIPE) || (tp == OR_OP) || (tp == AND_OP) || (tp == BG_PROC) ||
          (tp == NULL_TYPE));
}
